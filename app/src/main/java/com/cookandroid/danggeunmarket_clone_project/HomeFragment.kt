...
class HomeFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dbHelper = DBHelper(requireContext(), "resellData.db", null, 1)
    ...
    override fun onCreateView(
        ...
        var query = "SELECT * FROM resellData;"
        var cursor = database.rawQuery(query, null)
        while(cursor.moveToNext()){
            var id = cursor.getInt(cursor.getColumnIndexOrThrow("num"))
            var title = cursor.getString(cursor.getColumnIndexOrThrow("title"))
            var placetime = cursor.getString(cursor.getColumnIndexOrThrow("placetime"))
            ...
            var likeNum = cursor.getInt(cursor.getColumnIndexOrThrow("likeNum"))
            var resellData = ResellData(id, getDrawable(requireContext(), R.drawable.home_image_one)!!, title, placetime, content, cost, likeNum)
            resellList.add(resellData)
        }
//        for(i in 0..1) {
...
//        }
        ...
        val adapter = HomeAdapter(activity?.supportFragmentManager!!, requireContext())
